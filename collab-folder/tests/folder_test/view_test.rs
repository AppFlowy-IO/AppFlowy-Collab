...
use collab_folder::core::IconType;
...
async fn update_view_icon_test() {
  ...
  let icon = gen_view_icon(IconType::Emoji, "👍".to_string());
  folder_test
    .views
  ...
  let new_icon = gen_view_icon(IconType::Emoji, "👎".to_string());
  folder_test
    .views
    .update_view("v1", |update| {
...
async fn different_icon_ty_test() {
  ...
  let o_view = make_test_view("v1", "w1", vec![]);
  folder_test.insert_view(o_view, None);
  let emoji = gen_view_icon(IconType::Emoji, "👍".to_string());
  folder_test
    .views
  ...
  let icon = gen_view_icon(IconType::Icon, "👍".to_string());
  folder_test
    .views
    .update_view("v1", |update| update.set_icon(Some(icon.clone())).done())
    .unwrap();
