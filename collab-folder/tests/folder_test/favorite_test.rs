...
fn create_favorite_test() {
  ...
  let view_1 = folder_test.views.get_view(&txn, "1").unwrap();
  assert!(!view_1.is_favorite);
  folder_test.add_favorite_view_ids(&mut txn, vec!["1".to_string()]);
  ...
  let favorites = folder_test.get_my_favorite_sections(&txn);
...
fn add_favorite_view_and_then_remove_test() {
  ...
  folder_test.delete_favorite_view_ids(&mut txn, vec!["1".to_string()]);
  let views = folder_test.views.get_views_belong_to(&txn, &workspace_id);
  assert!(!views[0].is_favorite);
}
...
fn create_multiple_user_favorite_test() {
  ...
  folder_test_1.add_favorite_view_ids(&mut txn, vec!["1".to_string(), "2".to_string()]);
  let favorites = folder_test_1.get_my_favorite_sections(&txn);
  ...
  let favorites = folder_test2.get_my_favorite_sections(&txn);
...
fn favorite_data_serde_test() {
  ...
  folder_test.add_favorite_view_ids(&mut txn, vec!["1".to_string(), "2".to_string()]);
...
fn delete_favorite_test() {
  ...
  folder_test.remove_all_my_favorite_sections(&mut txn);
  let favorites = folder_test.get_my_favorite_sections(&txn);
...
fn migrate_favorite_v1_test() {
  ...
  folder_test.add_favorite_view_ids(
    &mut txn,
    favorites.into_iter().map(|fav| fav.id).collect::<Vec<_>>(),
  );
  folder_test.migrate_workspace_to_view(&mut txn);
