...
impl BlockOperation {
  ...
  pub fn set_block_with_txn(
    ...
    if let Some(parent_id) = parent_id {
      map.insert(txn, PARENT, parent_id);
    }
    // Update data field with the given data.
    if let Some(data) = data {
      map.insert(txn, DATA, hashmap_to_json_str(data)?);
    }

    // Update external id and external type.
    if let Some(external_id) = external_id {
      map.insert(txn, EXTERNAL_ID, external_id);
    }

    if let Some(external_type) = external_type {
      map.insert(txn, EXTERNAL_TYPE, external_type);
    }
    Ok(())
  }
}
