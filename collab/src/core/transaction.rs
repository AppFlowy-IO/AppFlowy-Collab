use crate::core::collab::CollabData;
...
pub struct LockedTransaction<'a> {
  #[allow(dead_code)]
  lock: RwLockReadGuard<'a, CollabData>,
  inner: Transaction<'a>,
}

impl<'a> LockedTransaction<'a> {
  pub(crate) fn new(lock: RwLockReadGuard<'a, CollabData>) -> Self {
...
pub struct LockedTransactionMut<'a> {
  #[allow(dead_code)]
  lock: RwLockWriteGuard<'a, CollabData>,
  inner: TransactionMut<'a>,
}

impl<'a> LockedTransactionMut<'a> {
  pub(crate) fn new(lock: RwLockWriteGuard<'a, CollabData>, origin: CollabOrigin) -> Self {
