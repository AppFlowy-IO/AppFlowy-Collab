...
use crate::database::entity::{DatabaseView, DatabaseViewMeta};

use crate::database::views::define::*;
use crate::database::views::layout::{DatabaseLayout, LayoutSettings};
...
impl OrderObjectPosition {
  ...
  pub fn after_row(row_id: &crate::entity::uuid_validation::RowId) -> Self {
    OrderObjectPosition::After(row_id.to_string())
  }

  /// Create a Before position with a RowId
  pub fn before_row(row_id: &crate::entity::uuid_validation::RowId) -> Self {
...
pub fn view_from_map_ref<T: ReadTxn>(map_ref: &MapRef, txn: &T) -> Option<DatabaseView> {
  ...
  Some(DatabaseView {
    id: crate::entity::uuid_validation::try_parse_database_view_id(&id)
      .unwrap_or_else(crate::entity::uuid_validation::generate_database_view_id),
    database_id: crate::entity::uuid_validation::try_parse_database_id(&database_id)
      .unwrap_or_else(crate::entity::uuid_validation::generate_database_id),
    name,
    layout,
    layout_settings,
    filters,
