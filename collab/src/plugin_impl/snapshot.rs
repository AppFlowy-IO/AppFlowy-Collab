...
impl CollabSnapshotPlugin {
  pub fn new(db: Arc<CollabKV>, snapshot_per_txn: u32) -> Result<Self, CollabError> {
...
impl CollabPlugin for CollabSnapshotPlugin {
  fn after_transaction(&self, cid: &str, txn: &mut TransactionMut) {
    let count = self.increase_count();
    if count != 0 && count % self.snapshot_per_txn == 0 {
      // generate snapshot
      if let Err(err) = self.snapshot().push_snapshot(cid, "".to_string(), txn) {
