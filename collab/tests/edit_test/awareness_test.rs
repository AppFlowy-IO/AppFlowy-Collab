...
async fn awareness_insert_test() {
  let mut collab = Collab::new(1, "1", "1", vec![], true, None);
...
async fn awareness_updates() {
  let mut c1 = Collab::new(1, "1", "1", vec![], true, None);
  c1.emit_awareness_state();
  let mut c2 = Collab::new(2, "1", "1", vec![], true, None);
...
async fn clean_awareness_state_sync_test() {
  ...
  let mut collab_1 = Collab::new(0, "1", "1", vec![], true, None);
  ...
  let mut collab_2 = Collab::new(1, "1", "2", vec![], true, None);
