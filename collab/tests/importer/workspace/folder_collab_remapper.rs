...
use collab::folder::{Folder, ViewLayout};
use collab::importer::workspace::folder_collab_remapper::FolderCollabRemapper;
...
fn verify_view(
  ...
  assert_eq!(
    view.parent_view_id,
    Some(collab::entity::uuid_validation::view_id_from_any_string(
      expected_parent_id
    ))
  );
...
async fn test_folder_collab_remapper() {
  ...
  for view in &all_views {
    ...
    for (i, child) in view.children.iter().enumerate() {
      ...
      assert_eq!(
        child.id,
        collab::entity::uuid_validation::view_id_from_any_string(&expected_child_id.to_string())
      );
    }
  }
}
