...
use collab::document::blocks::{Block, BlockAction, DocumentData, DocumentMeta};
...
use collab::plugins::local_storage::rocksdb::util::KVDBCollabPersistenceImpl;
...
pub fn unzip_history_document_db(folder_name: &str) -> std::io::Result<(Cleaner, PathBuf)> {
  // Open the zip file
  let zip_file_path = format!("./tests/document/history_document/{}.zip", folder_name);
  let reader = File::open(zip_file_path)?;
  let output_folder_path = format!("./tests/document/history_document/unit_test_{}", nanoid!(6));
...
pub fn try_decode_from_encode_collab(document: &Document) {
  ...
  let options =
    CollabOptions::new(Uuid::new_v4(), default_client_id()).with_data_source(data.into());
