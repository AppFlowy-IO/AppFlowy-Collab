...
use collab::database::{fields::Field, views::OrderObjectPosition};
use uuid::Uuid;
...
async fn create_field_in_view_test() {
  ...
  let fields = database_test.get_fields_in_view(TEST_VIEW_ID_V1, None);
  assert_eq!(fields[0].id, "f0");
  assert_eq!(fields[1].id, "f1");
  assert_eq!(fields[2].id, "f2");

  let fields = database_test.get_fields_in_view(TEST_VIEW_ID_V2, None);
  assert_eq!(fields[0].id, "f0");
  assert_eq!(fields[1].id, "f1");
  assert_eq!(fields[2].id, "f2");

  database_test.create_field(
    Some(TEST_VIEW_ID_V2),
