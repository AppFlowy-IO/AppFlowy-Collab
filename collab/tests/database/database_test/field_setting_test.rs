...
use collab::database::views::{DatabaseLayout, OrderObjectPosition};
...
async fn new_field_new_field_setting_test() {
  ...
  let field_settings_map: HashMap<String, TestFieldSetting> =
    database_test.get_field_settings(TEST_VIEW_ID_V1, None);
  assert_eq!(field_settings_map.len(), 4);

  let field_settings_map: HashMap<String, TestFieldSetting> =
    database_test.get_field_settings(TEST_VIEW_ID_V2, None);
  assert_eq!(field_settings_map.len(), 4);
}
...
async fn update_field_setting_for_some_fields_test() {
  ...
  database_test.update_field_settings(TEST_VIEW_ID_V1, None, field_settings);
}
...
async fn duplicate_view_duplicates_field_settings_test() {
  ...
  let field_settings_map: HashMap<String, TestFieldSetting> =
    database_test.get_field_settings(&duplicate_view.id.to_string(), None);
  let test_field_settings = field_settings_map.get("f1").unwrap();
