...
use anyhow::bail;
use collab::database::fields::{TypeOptionData, TypeOptionDataBuilder};
...
pub fn unzip_history_database_db(folder_name: &str) -> std::io::Result<(Cleaner, PathBuf)> {
  // Open the zip file
  let zip_file_path = format!("./tests/database/history_database/{}.zip", folder_name);
  let reader = File::open(zip_file_path)?;
  let output_folder_path = format!("./tests/database/history_document/unit_test_{}", nanoid!(6));
