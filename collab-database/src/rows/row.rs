...
impl Row {
  /// Returns the document_id of the row.
  /// The document_id is unique for each row.
  pub fn document_id(&self) -> String {
    match Uuid::parse_str(self.id.as_str()) {
      Ok(row_id) => document_id_from_row_id(&row_id),
      Err(e) => {
...
pub fn document_id_from_row_id(row_id: &Uuid) -> String {
  Uuid::new_v5(&row_id, b"document_id").to_string()
}
