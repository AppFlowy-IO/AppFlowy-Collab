...
async fn open_020_history_database_test() {
  ...
  // the linked view is created from the inline view
  let expected_linked_view = DatabaseView {
    id: data.views[0].id.clone(),
    created_at: data.views[0].created_at,
    modified_at: data.views[0].modified_at,
    ..inline_view
  };
  assert_eq!(data.views[0], expected_linked_view);
}
