...
async fn get_layout_setting_test() {
  ...
  let layout_setting = database_test
    .get_layout_setting::<TestCalendarLayoutSetting>(TEST_VIEW_ID_V1, &DatabaseLayout::Grid)
    .unwrap();
...
async fn create_database_view_with_layout_setting_test() {
  ...
  let layout_setting = database_test
    .get_layout_setting::<TestCalendarLayoutSetting>(TEST_VIEW_ID_V1, &DatabaseLayout::Calendar)
    .unwrap();
  assert_eq!(layout_setting.field_id, "f1");
}
...
async fn update_layout_setting_test() {
  ...
  database_test.insert_layout_setting(TEST_VIEW_ID_V1, &DatabaseLayout::Board, layout_setting);
...
async fn create_database_with_two_layout_settings() -> DatabaseTest {
  ...
  database_test.insert_layout_setting(TEST_VIEW_ID_V1, &DatabaseLayout::Board, layout_setting_1);
  database_test.insert_layout_setting(TEST_VIEW_ID_V1, &DatabaseLayout::Grid, layout_setting_2);
