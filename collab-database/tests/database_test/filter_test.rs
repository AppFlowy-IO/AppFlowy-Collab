...
async fn create_database_view_with_filter_test() {
  ...
  let filter_1 = database_test
    .get_filter::<TestFilter>(TEST_VIEW_ID_V1, "filter_1")
    .unwrap();
  assert_eq!(filter_1.content, "hello filter");

  let filter_2 = database_test
    .get_filter::<TestFilter>(TEST_VIEW_ID_V1, "filter_2")
    .unwrap();
...
async fn insert_or_update_database_view_filter_test() {
  ...
  // Update
  database_test.update_filter(TEST_VIEW_ID_V1, "filter_1", |update| {
...
async fn insert_database_view_filter_to_filtering_field_id_test() {
  ...
  let filter_3 = database_test
    .get_filter::<TestFilter>(TEST_VIEW_ID_V1, "filter_3")
    .unwrap();
...
async fn remove_database_view_filter_test() {
  let mut database_test = create_database_with_two_filters().await;
  database_test.remove_filter(TEST_VIEW_ID_V1, "filter_1");
  let filter_1 = database_test.get_filter::<TestFilter>(TEST_VIEW_ID_V1, "filter_1");
...
async fn create_database_with_two_filters() -> DatabaseTest {
  ...
  database_test.insert_filter(TEST_VIEW_ID_V1, filter_1);
  database_test.insert_filter(TEST_VIEW_ID_V1, filter_2);
