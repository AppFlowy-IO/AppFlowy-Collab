...
pub trait Schema {
  ...
  fn validate<T: ReadTxn>(tx: &T) -> Result<(), CollabValidateError> {
    let schema = Self::schema();
    match &schema {
      Value::Object(map) => {
        for (field, v) in map {
...
mod test {
  ...
  fn test_schema() {
    ...
    TestSchema::init(&mut tx);
    assert_matches!(TestSchema::validate(&tx), Ok(_));
    assert_matches!(TestSchema2::validate(&tx), Err(_));
  }
}
